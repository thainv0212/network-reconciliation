...
class Algorithm:
    ...
    def sim_local(self, g1_node: int, g2_node: int):
        g1_node_emb = g1.node_embedding[g1_node]
        g2_node_emb = g2.node_embedding[g2_node]
        # tmp = g1_node_emb - g2_node_emb
        # return 0 - (tmp ** 2).sum()
        return (g1_node_emb * g2_node_emb).sum()/np.sqrt((g1_node_emb ** 2).sum() * (g2_node_emb **  2).sum())
...
def read_node_embedding(graph_file):
    with open(graph_file, 'r') as f:
        meta_data = f.readline().split(' ')
        node_num = int(meta_data[0])
        emb_dim = int(meta_data[1])
        node_emb = np.zeros((node_num, emb_dim))
        for i in range(node_num):
            edge_data = f.readline()
            splits = edge_data.split(' ')
            node_id = int(splits[0])
            emb = np.array([float(i) for i in splits[1:]])
            node_emb[i,:] = emb
    return node_emb
...
g1_embedding = read_node_embedding('data_10000/{}/graph_0_emb.txt'.format(argv[1]))
g1.node_embedding = g1_embedding
g2 = Graph.load('data_10000/{}/graph_1.pickle'.format(argv[1]))
g2_embedding = read_node_embedding('data_10000/{}/graph_1_emb.txt'.format(argv[1]))
g2.node_embedding = g2_embedding

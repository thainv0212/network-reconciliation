...
for pes in rates:
    pe1 = pes[0]
    pe2 = pes[1]
    print('processing data with pe1 = {} and pe2 = {}'.format(pe1, pe2))
    # g1, node_alias_1, node_values_1 = gen_subgraph(link_data_df, pe1)
    # g2, node_alias_2, node_values_2 = gen_subgraph(link_data_df, pe2)
    # g1, node_alias_1, node_values_1 = gen_subgraph_new(link_data_df, pe1)
    # g2, node_alias_2, node_values_2 = gen_subgraph_new(link_data_df, pe2)
    g1, node_alias_1, node_values_1 = gen_subgraph_new_2(link_data_df, pe1)
    g2, node_alias_2, node_values_2 = gen_subgraph_new_2(link_data_df, pe2)
    # g3, node_alias_3, node_values_3 = gen_subgraph_new(link_data_df, 1)
    print('start globalrank computation process')
    for idx, (g, node_alias, node_values) in enumerate(zip((g1, g2), (node_alias_1, node_alias_2), (node_values_1, node_values_2))):
        ...
        parent = 'data_{}'.format(limit)
        if not path.exists(parent):
            mkdir(parent)
        dir_name = 'data_{}/{}_{}'.format(limit,pe1, pe2)
        if not path.exists(dir_name):
            mkdir(dir_name)
        graph.save(path.join(dir_name,'graph_{}.pickle'.format(idx)))
